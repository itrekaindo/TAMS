...
@section('content')
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div
                class="group relative bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100">
                <div
                    class="absolute top-0 right-0 w-24 h-24 bg-red-500/5 rounded-full -mr-12 -mt-12 group-hover:scale-150 transition-transform duration-500">
                </div>
                <div class="relative p-6">
                    <div class="flex items-center justify-between mb-3">
                        <div
                            class="w-12 h-12 bg-gradient-to-br from-red-500 to-pink-500 rounded-xl flex items-center justify-center shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300">
                    ...
                    <p class="text-3xl font-black text-gray-900 mb-1">
                        {{ \App\Models\Peminjaman::where('status', 'dipinjam')->where('tanggal_kembali_rencana', '<', now())->count() }}
                    </p>
                    <p class="text-sm text-gray-500 font-medium">Melewati Deadline</p>
                </div>
                <div
                    class="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-red-500 to-pink-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500">
                </div>
            </div>
        </div>
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <select id="filterStatus"
                            class="px-8 py-2.5 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm font-medium transition-all hover:border-gray-300">
            ...
            <div class="overflow-x-auto">
                @if ($peminjamans->isEmpty())
                    <div class="text-center py-20">
                        ...
                        <p class="text-gray-500 mb-6 max-w-md mx-auto">Mulai dengan membuat transaksi peminjaman baru untuk
                            mencatat aktivitas peminjaman alat</p>
                        <a href="{{ route('peminjaman.create') }}"
                            ...
                            @foreach ($peminjamans as $peminjaman)
                                <tr
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-2">
                                            <div
                                                class="w-8 h-8 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-lg flex items-center justify-center">
                                                <i class="bi bi-hash text-blue-600 text-sm"></i>
                                            </div>
                                            <code
                                                class="text-xs font-mono font-bold bg-gray-100 px-2.5 py-1.5 rounded-lg text-gray-700 group-hover:bg-blue-100 group-hover:text-blue-700 transition-colors">
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-3">
                                            <div class="relative">
                                                <div
                                                    class="w-11 h-11 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center text-white font-black text-base shadow-lg group-hover:scale-110 transition-transform duration-300">
                                            ...
                                            <div class="min-w-0">
                                                <p class="font-bold text-gray-900 truncate">{{ $peminjaman->nama_lengkap }}
                                                </p>
                                        ...
                                        @if ($peminjaman->details && $peminjaman->details->count() > 0)
                                            <div class="space-y-2">
                                                @foreach ($peminjaman->details->take(2) as $detail)
                                                    <div class="flex items-center gap-2">
                                                        <span
                                                            class="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-bold rounded-lg bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100 transition-colors">
                                                            <i class="bi bi-box"></i>
                                                            <span
                                                                class="max-w-[180px] truncate">{{ $detail->nama_alat }}</span>
                                                        </span>
                                                        <span
                                                            class="inline-flex items-center px-2 py-1 text-xs font-black rounded-lg bg-gradient-to-r from-gray-100 to-gray-200 text-gray-700">
                                                            ×{{ $detail->jumlah }}
                                                        </span>
                                                    </div>
                                                @endforeach
                                                ...
                                                @if ($peminjaman->details->count() > 2)
                                                    <button onclick="showAllItems({{ $peminjaman->id }})"
                                ...
                                @endif
                                    ...
                                    <td class="px-6 py-5 text-center">
                                        ...
                                        @if ($peminjaman->status === 'dikembalikan')
                                            <span
                                                class="inline-flex items-center gap-2 px-4 py-2 text-xs font-black rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg shadow-green-500/30">
                                                <i class="bi bi-check-circle-fill"></i>
                                                Dikembalikan
                                            </span>
                                        @elseif($peminjaman->status === 'sebagian_dikembalikan')
                                            <span
                                                class="inline-flex items-center gap-2 px-4 py-2 text-xs font-black rounded-xl bg-gradient-to-r from-blue-500 to-cyan-500 text-white shadow-lg shadow-blue-500/30">
                                                <i class="bi bi-arrow-repeat"></i>
                                                Dikembalikan: {{ $alatKembali }}/{{ $totalAlat }}
                                            </span>
                                        @elseif($peminjaman->isLate())
                                            <span
                                                class="inline-flex items-center gap-2 px-4 py-2 text-xs font-black rounded-xl bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-lg shadow-red-500/30 animate-pulse">
                                                <i class="bi bi-exclamation-circle-fill"></i>
                                                Terlambat
                                            </span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-2 px-4 py-2 text-xs font-black rounded-xl bg-gradient-to-r from-yellow-400 to-orange-400 text-white shadow-lg shadow-yellow-500/30">
                                                <i class="bi bi-clock-fill"></i>
                                                Dipinjam
                                            </span>
                                        @endif
    ...
    <div id="itemsModal"
        ...
        <div class="bg-white rounded-3xl shadow-2xl max-w-3xl w-full max-h-[85vh] overflow-hidden transform transition-all"
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 font-medium mt-0.5">Rincian lengkap alat dalam peminjaman ini
                            </p>
                        </div>
                    </div>
                    <button onclick="closeModal()"
                        class="w-10 h-10 flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-xl transition-all">

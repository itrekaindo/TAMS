...
@section('title', 'Edit Alat - ' . $alat->nama_alat)
@section('page-title', 'Edit Alat')
@section('page-subtitle', 'Perbarui informasi alat yang sudah terdaftar')
...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm">
        ...
        <i class="bi bi-chevron-right text-gray-400 text-xs"></i>
        <span class="text-gray-900 font-semibold">Edit Alat</span>
    </nav>
        ...
        <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
            ...
            <div class="bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 p-8">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-3xl font-black text-white mb-1">Form Edit Alat</h2>
                        <p class="text-amber-100 font-medium">Perbarui informasi untuk: <span class="font-black">{{ $alat->nama_alat }}</span></p>
                    </div>
                </div>
            </div>

            {{-- Current Info Card --}}
            <div class="bg-gradient-to-r from-amber-50 to-orange-50 border-b-2 border-amber-200 p-6">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="text-center p-3 bg-white rounded-xl shadow-sm overflow-hidden">
                        <p class="text-xs text-gray-500 font-bold mb-1 truncate">Kode Alat</p>
                        <p class="text-lg font-black text-gray-900 truncate">{{ $alat->kode_alat ?: '—' }}</p>
                    </div>
                    <div class="text-center p-3 bg-white rounded-xl shadow-sm">
                        <p class="text-xs text-gray-500 font-bold mb-1">Total Unit</p>
                        <p class="text-lg font-black text-blue-600">{{ $alat->jumlah_total }}</p>
                    </div>
                    <div class="text-center p-3 bg-white rounded-xl shadow-sm">
                        <p class="text-xs text-gray-500 font-bold mb-1">Tersedia</p>
                        <p class="text-lg font-black text-green-600">{{ $alat->jumlah_tersedia }}</p>
                    </div>
                    <div class="text-center p-3 bg-white rounded-xl shadow-sm">
                        <p class="text-xs text-gray-500 font-bold mb-1">Dipinjam</p>
                        <p class="text-lg font-black text-orange-600">{{ $alat->jumlah_total - $alat->jumlah_tersedia }}</p>
                    </div>
                </div>
            </div>
            ...
            {{-- Form Content --}}
            <div class="p-8">
                <form action="{{ route('alat.update', $alat->id) }}" method="POST" id="editAlatForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="kode_alat" class="flex items-center gap-2 text-sm font-bold text-gray-700 mb-2">
                                    <i class="bi bi-upc-scan text-amber-600"></i>
                                    Kode Alat
                                    {{-- ❌ Hapus tanda bintang --}}
                                </label>
                                <div class="relative">
                                    <input
                                        ...
                                        {{-- ❌ Hapus required --}}
                                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all font-mono text-sm @error('kode_alat') border-red-500 @enderror"
                                    >
                                    @error('kode_alat')
                                ...
                                <p class="mt-2 text-xs text-gray-500 flex items-center gap-1">
                                    ...
                                    Sebelumnya: <span class="font-bold">{{ $alat->kode_alat ?: 'Tidak ada' }}</span>
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input
                                        ...
                                        required
                                        class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all @error('nama_alat') border-red-500 @enderror"
                                    >
                                    @error('nama_alat')
                                        <div class="absolute -bottom-6 left-0 flex items-center gap-1 text-xs text-red-600">
                                            <i class="bi bi-exclamation-circle-fill"></i>
                        ...
                        <div class="mt-8">
                            ...
                            <textarea
                                ...
                                class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-amber-500 outline-none transition-all resize-none @error('deskripsi') border-red-500 @enderror"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    value="{{ old('spesifikasi_type', $alat->spesifikasi_type) }}"
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    id="kapasitas"
                                    name="kapasitas"
                                    value="{{ old('kapasitas', $alat->kapasitas) }}"
                                    placeholder="Contoh: 10L, 500W, 200kg"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                @if($alat->kategori)
                                    <p class="mt-2 text-xs text-gray-500 flex items-center gap-1">
                                        <i class="bi bi-info-circle"></i>
                                        Sebelumnya: <span class="font-bold">{{ $alat->kategori }}</span>
                                    </p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                @if($alat->lokasi)
                                    <p class="mt-2 text-xs text-gray-500 flex items-center gap-1">
                                        <i class="bi bi-info-circle"></i>
                                        Sebelumnya: <span class="font-bold">{{ $alat->lokasi }}</span>
                                    </p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach(\App\Models\Alat::jenisToolsOptions() as $key => $value)
                                        <option value="{{ $key }}" {{ old('jenis_tools', $alat->jenis_tools) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                @if($alat->jenis_tools)
                                    <p class="mt-2 text-xs text-gray-500">
                                        Sebelumnya: <span class="font-bold">{{ $alat->jenis_tools }}</span>
                                    </p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach(\App\Models\Alat::kategoriToolsOptions() as $key => $value)
                                        <option value="{{ $key }}" {{ old('kategori_tools', $alat->kategori_tools) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                @if($alat->kategori_tools)
                                    <p class="mt-2 text-xs text-gray-500">
                                        Sebelumnya: <span class="font-bold">{{ $alat->kategori_tools }}</span>
                                    </p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    @foreach(\App\Models\Alat::proyekOptions() as $key => $value)
                                        <option value="{{ $key }}" {{ old('proyek', $alat->proyek) == $key ? 'selected' : '' }}>
                                            {{ $value }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                @if($alat->proyek)
                                    <p class="mt-2 text-xs text-gray-500">
                                        Sebelumnya: <span class="font-bold">{{ $alat->proyek }}</span>
                                    </p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    id="pemakai"
                                    name="pemakai"
                                    value="{{ old('pemakai', $alat->pemakai) }}"
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    value="{{ old('lokasi_distribusi', $alat->lokasi_distribusi) }}"
                            ...
                            <div class="lg:col-span-2">
                                <div class="flex items-start gap-3 pt-2">
                                    <input
                                        ...
                                        name="sticker"
                                        value="1"
                                        {{ (old('sticker', $alat->sticker)) ? 'checked' : '' }}
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">
                                            Status sebelumnya: <span class="font-bold">{{ $alat->sticker ? 'Ya' : 'Tidak' }}</span>
                                        </p>
                                    </div>
                                </div>
                                @error('sticker')
                            ...
                            <div class="lg:col-span-2">
                                <div class="flex items-start gap-3 pt-2">
                                    <input
                                        ...
                                        name="hilang"
                                        value="1"
                                        {{ (old('hilang', $alat->hilang)) ? 'checked' : '' }}
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">
                                            Status sebelumnya: <span class="font-bold">{{ $alat->hilang ? 'Hilang' : 'Tersedia' }}</span>
                                        </p>
                                    </div>
                                </div>
                                @error('hilang')
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input
                                        ...
                                        min="{{ $alat->jumlah_total - $alat->jumlah_tersedia }}"
                                        required
                                ...
                                <p class="mt-2 text-xs text-gray-500 flex items-center gap-1">
                                    <i class="bi bi-info-circle"></i>
                                    Minimal: <span class="font-bold text-orange-600">{{ $alat->jumlah_total - $alat->jumlah_tersedia }}</span> (sedang dipinjam)
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="mt-2 text-xs text-gray-500 flex items-center gap-1">
                                    <i class="bi bi-info-circle"></i>
                                    Sebelumnya: <span class="font-bold">{{ \App\Models\Alat::kondisiOptions()[$alat->kondisi] ?? $alat->kondisi }}</span>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Info Box Stok --}}
                    <div class="mb-8 p-6 bg-gradient-to-r from-amber-50 via-orange-50 to-red-50 border-2 border-amber-200 rounded-2xl">
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1">
                                <p class="font-black text-amber-900 mb-3 text-lg">Informasi Penting!</p>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                                    <div class="bg-white p-3 rounded-xl shadow-sm">
                                        <p class="text-xs text-gray-500 font-bold mb-1">Jumlah Total</p>
                                        <p class="text-2xl font-black text-blue-600">{{ $alat->jumlah_total }}</p>
                                        <p class="text-xs text-gray-500 mt-1">unit</p>
                                    </div>
                                    <div class="bg-white p-3 rounded-xl shadow-sm">
                                        <p class="text-xs text-gray-500 font-bold mb-1">Tersedia</p>
                                        <p class="text-2xl font-black text-green-600">{{ $alat->jumlah_tersedia }}</p>
                                        <p class="text-xs text-gray-500 mt-1">unit</p>
                                    </div>
                                    <div class="bg-white p-3 rounded-xl shadow-sm">
                                        <p class="text-xs text-gray-500 font-bold mb-1">Dipinjam</p>
                                        <p class="text-2xl font-black text-orange-600">{{ $alat->jumlah_total - $alat->jumlah_tersedia }}</p>
                                        <p class="text-xs text-gray-500 mt-1">unit</p>
                                    </div>
                                </div>
                                <ul class="space-y-2 text-sm text-amber-800">
                                    <li class="flex items-start gap-2">
                                        <i class="bi bi-check-circle-fill text-amber-600 mt-0.5"></i>
                                        <span>Jumlah total tidak boleh kurang dari jumlah yang sedang dipinjam</span>
                                    </li>
                                    <li class="flex items-start gap-2">
                                        <i class="bi bi-check-circle-fill text-amber-600 mt-0.5"></i>
                                        <span>Jumlah tersedia akan disesuaikan otomatis setelah update</span>
                                    </li>
                                    <li class="flex items-start gap-2">
                                        <i class="bi bi-check-circle-fill text-amber-600 mt-0.5"></i>
                                        <span>Perubahan akan mempengaruhi ketersediaan untuk peminjaman</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="flex flex-wrap gap-4 pt-6 border-t-2 border-gray-100">
                        <button
                            type="submit"
                            class="group inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-amber-600 to-orange-600 text-white rounded-xl hover:shadow-lg hover:shadow-amber-500/50 transition-all font-bold hover:-translate-y-0.5">
...
@section('scripts')
<script>
    // Auto-generate kode alat suggestion (opsional, tetap berjalan)
    ...
    const minValue = {{ $alat->jumlah_total - $alat->jumlah_tersedia }};
    jumlahTotalInput?.addEventListener('change', function() {
        if (parseInt(this.value) < minValue) {
            this.value = minValue;
            alert(`Jumlah total tidak boleh kurang dari ${minValue} unit (sedang dipinjam)!`);
        }
    });

    // Input validation styling — hanya untuk field required
    const inputs = document.querySelectorAll('input[name="nama_alat"], input[name="jumlah_total"], select[name="kondisi"]');
    ...
    // Form submit
    document.getElementById('editAlatForm')?.addEventListener('submit', function(e) {

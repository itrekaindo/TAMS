        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div
                ...
                <div class="relative p-6">
                    ...
                    <p class="text-3xl font-black text-gray-900 mb-1">
                        {{ \App\Models\Alat::whereIn('kondisi', ['rusak','rusak_ringan','rusak_berat'])->count() }}
                    </p>
            ...
            <div class="px-6 py-5 bg-gradient-to-r from-gray-50 to-purple-50 border-b border-gray-100">
                <form method="GET" action="{{ route('alat.index') }}">
                    ...
                    <div class="flex gap-3 mt-4">
                        ...
                        @if (request()->anyFilled(['search', 'kondisi', 'kategori', 'lokasi']))
            ...
            <div class="overflow-x-auto">
                @if ($alats->isEmpty())
                    <div class="text-center py-20">
                        ...
                        <p class="text-gray-500 mb-6 max-w-md mx-auto">
                            @if (request()->anyFilled(['search', 'kondisi', 'kategori']))
                                Tidak ditemukan alat yang sesuai dengan filter. Silakan coba reset filter atau gunakan kata
                                kunci lain.
                            @else
                ...
                @else
                    <table class="min-w-full">
                        <thead>
                            <tr class="bg-gray-50/50 border-b border-gray-100" style="align-items: center">
                                <th
                                    class="px-6 py-4 text-center text-xs font-black text-gray-600 uppercase tracking-wider">
                                    Informasi Alat
                                </th>
                                <th
                                    class="px-6 py-4 text-center text-xs font-black text-gray-600 uppercase tracking-wider">
                                ...
                                <tr
                                    ...
                                    <td class="px-6 py-5">
                                        <div class="flex items-center gap-4">
                                            <div class="relative">
                                                <div
                                                    class="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform duration-300">
                                                ...
                                                <div class="flex items-center gap-2 mt-1 flex-wrap">
                                                    ...
                                                    @if ($alat->kategori)
                                                        @php
                                                            ...
                                                            $badge = $kategoriBadge[$alat->kategori] ?? [
                                                                'bg' => 'bg-gray-100',
                                                                'text' => 'text-gray-700',
                                                                'icon' => 'bi-tag-fill'
                                                            ];
                                                        ...
                                                        <span class="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 {{ $badge['bg'] }} {{ $badge['text'] }} rounded-lg">
                                        ...
                                        <div class="flex flex-col gap-2">
                                            @if ($alat->merk)
                                                <div class="flex items-center gap-2">
                                                    <div
                                                        ...
                                                        <i class="bi bi-badge-tm-fill text-amber-600 text-sm"></i>
                                                    </div>
                                                    <div class="min-w-0">
                                                        <p class="text-xs text-gray-500 font-medium">Merk</p>
                                            ...
                                            @if ($alat->spesifikasi_type)
                                                <div class="flex items-center gap-2">
                                                    ...
                                                    <div class="min-w-0">
                                                        <p class="text-xs text-gray-500 font-medium">Spesifikasi</p>
                                                        <p class="text-sm font-bold text-gray-900 truncate">
                                                            {{ $alat->spesifikasi_type }}</p>
                                                    </div>
                                                </div>
                                            @endif

                                            @if (!$alat->merk && !$alat->spesifikasi_type)
                                                <div class="flex items-center gap-2 text-gray-400">
                                                    <i class="bi bi-dash-circle text-lg"></i>
                                                    <span class="text-sm italic">Tidak ada data</span>
                                                </div>
                                            @endif
                                        </div>
                                    </td>

                                    {{-- Stok → STATUS: Tersedia / Tidak Tersedia --}}
                                    {{-- Stok --}}
                                    <td class="px-6 py-5">
                                        <div class="flex flex-col items-center justify-center h-full">
                                            @if ($alat->hilang)
                                                <span
                                                    class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-black bg-gradient-to-r from-red-600 to-pink-700 text-white shadow-lg shadow-red-500/50 animate-pulse">
                                                    <i class="bi bi-exclamation-triangle-fill text-xs"></i>
                                                    Hilang
                                                </span>
                                            @elseif($alat->jumlah_tersedia > 0)
                                                <span
                                                    class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-black bg-gradient-to-r from-green-500 to-emerald-500 text-white shadow-lg shadow-green-500/30">
                                                    <i class="bi bi-check-circle-fill text-xs"></i>
                                                    Tersedia
                                                </span>
                                            @else
                                                <span
                                                    class="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-black bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-lg shadow-red-500/30">
                                    ...
                                    <td class="px-6 py-5">
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black bg-gradient-to-r {{ $badge['gradient'] }} text-white shadow-lg {{ $badge['shadow'] }}">
                                            <i class="bi {{ $badge['icon'] }}"></i>
                                    ...
                                    <td class="px-6 py-5">
                                        @if ($alat->lokasi)
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span
                                                    class="text-sm font-semibold text-gray-700">{{ $alat->lokasi }}</span>
                                            </div>
                                        @else
                                            <div class="flex items-center gap-2 text-gray-400">
                                    ...
                                    <td class="px-6 py-5">
                                        @if ($alat->pic)
                                            <div class="flex items-center gap-2">
                                                ...
                                                <span
                                                    class="text-sm font-semibold text-gray-700">{{ $alat->pic }}</span>
                                            </div>
                                        @else
                                            <div class="flex items-center gap-2 text-gray-400">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div
                ...
                <div class="relative">
                    ...
                    <div class="text-5xl font-black text-white">{{ \App\Models\Alat::where('kondisi', 'baik')->count() }}
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div class="relative">
                    ...
                    <div class="text-5xl font-black text-white">
                        {{ \App\Models\Alat::distinct('kategori')->count('kategori') }}</div>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    document.querySelectorAll('select[name="kondisi"], select[name="kategori"], select[name="lokasi"]').forEach(select => {
